...
const pName = document.querySelector("#Category-name");
...
function onClick() {
    if (!b) {
        ...
        if (pName.value == "" || pName.value == " " || pName.value == 0 && description.value == "" || description.value == " " || description.value == 0) {
            setError(pName);
            return;
        }
        startUploading();
...
function startUploading() {
    ...
    let storageRef = ref(storage, "images/playlist-images/" + pName.value + ". " + image.files[0].name.split(".").pop());
